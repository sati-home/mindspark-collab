...
It implements the existing MindSpark client contract as a drop-in for the upstream Cloudflare worker. The bundled app carries one small client patch (`docker/client-collab.patch`) - the change this project proposes upstream, which decouples collaboration from GitHub OAuth; once that lands upstream, an unmodified MindSpark build only needs to be pointed at this backend.
